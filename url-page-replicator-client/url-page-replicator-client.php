...
define( 'UPR_CLIENT_PATH', plugin_dir_path( __FILE__ ) );
define( 'UPR_CLIENT_URL', plugin_dir_url( __FILE__ ) );
...
function upr_client_serve_page_template( $template ) {
	if ( is_page() ) {
		...
		if ( $is_replicated ) {
			$custom_template = UPR_CLIENT_PATH . 'templates/page-renderer.php';
...
function upr_client_enqueue_assets( $hook ) {
	...
	wp_enqueue_style( 'upr-client-admin-css', UPR_CLIENT_URL . 'assets/admin.css', array(), time() );
	
	wp_enqueue_script(
		'upr-client-admin-js',
		UPR_CLIENT_URL . 'assets/admin.js',
		array( 'jquery' ),
		time(),
		true
	);
							...
							<tr>
								...
								<td>
									<div class="upr-row-actions">
										<a href="<?php echo esc_url( $replica_url ); ?>" target="_blank" class="upr-action-btn upr-view-btn" title="<?php esc_attr_e( 'View Page', 'url-page-replicator-client' ); ?>">
											<span class="dashicons dashicons-visibility"></span>
										</a>
										<a href="<?php echo esc_url( admin_url( 'admin-ajax.php?action=upr_export_page&id=' . $replica->ID . '&nonce=' . wp_create_nonce( 'upr_client_nonce' ) ) ); ?>" class="upr-action-btn upr-export-btn" title="<?php esc_attr_e( 'Export Page', 'url-page-replicator-client' ); ?>">
		...
		<div class="upr-grid">
			...
			<section class="upr-card upr-config-card">
				...
				<?php else : ?>
					...
					<h2><?php esc_html_e( 'Import Replicated Page', 'url-page-replicator-client' ); ?></h2>
					<form id="upr-import-form" enctype="multipart/form-data">
						<div class="upr-form-group">
							<label for="import_file"><?php esc_html_e( 'Select Exported ZIP Package', 'url-page-replicator-client' ); ?></label>
							...
							<p class="upr-desc"><?php esc_html_e( 'Upload a previously exported .zip package to import the page and its assets.', 'url-page-replicator-client' ); ?></p>
						</div>
						<button type="submit" class="upr-btn-submit" style="background: linear-gradient(135deg, #10b981 0%, #059669 100%);">
							<span class="btn-text"><?php esc_html_e( 'Import Page Package', 'url-page-replicator-client' ); ?></span>
						</button>
					</form>
